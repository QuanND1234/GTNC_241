...
class BKTree:
    ...
    def search_word(self, word, max_distance=2):
        results = []
        if self.root:
    ...
    def _search_word(self, word, node, results, max_distance=2):
        ...
        for d in range(max(0, distance - max_distance), distance + max_distance + 1):
            if d in node.children:
                self._search_word(word, node.children[d], results, max_distance=max_distance)
...
def all():
    ...
    query_word = "ops"
    matches = tree.search_word(query_word)
    
    print(f"Words similar to '{query_word}' within distance {max_dist}: {matches}")

    query_word = "helt"
    matches = tree.search_word(query_word, max_dist)
